...
async function sortArrayJsonByDate(arrayJson) {
  arrayJson.sort((a, b) => {
    return (
      new Date(b.fecha_creacion_monturas) - new Date(a.fecha_creacion_monturas)
    ); // ascending order
  });
  return arrayJson;
}
...
export const getAllMonturasForVenta = async (req, res) => {
  ...
  try {
    ...
    const params = {
      ...
      FilterExpression:
        '#habilitado = :valueHabilitado and #cantidad > :valueCantidad and #id_sede = :valueSede',
...
export const createNewMontura = async (req, res) => {
  ...
  try {
    ...
    const newMontura = await dynamoClient
      .put({
        TableName: TABLE_NAME_MONTURAS,
        Item: datosMontura,
      })
...
const validateMontura = async (idMontura) => {
  ...
  try {
    const paramsMontura = {
      TableName: TABLE_NAME_MONTURAS,
      KeyConditionExpression: 'id_producto = :id_montura',
      ExpressionAttributeValues: {
...
export const editMonturaById = async (req, res) => {
  ...
  if (idSedeDestino !== id_sede) {
    const objeto = {
      ...
      sede_anterior: id_sede,
      sede_nueva: idSedeDestino,
      fecha_traslado: fecha_actual,
    };
