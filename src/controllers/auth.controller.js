...
async function findUserByEmail(usuario) {
  try {
    const paramsUsuario = {
      TableName: TABLE_NAME_USUARIO,
      FilterExpression: 'usuario = :usuario and habilitado = :estado',
      ExpressionAttributeValues: {
        ':usuario': usuario,
        ':estado': true,
      },
...
export const signIn = async (req, res) => {
  ...
  //Expire Token
  const expiresIn =
    user.Items[0].rol === ADMIN_ROLE ? TIME_EXPIRE_ADMIN : TIME_EXPIRE_OTHERS;

  const token = jwt.sign({ _id: user.Items[0].id_usuario }, 'secretkey', {
    expiresIn: expiresIn,
  });
